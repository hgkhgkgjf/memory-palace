...
- **检索**：只有 Profile B 默认开启 RRF 融合（`RRF_K=10`）。C/D 默认关闭 RRF 和 MMR，并在 pip 安装了 `sqlite-vec` 时走 sqlite-vec 原生向量引擎，没装则自动回退 legacy scoring。中英混合处理改进、全角归一化（`ＡＰＩ → API`）、MMR 去重、embedding 漂移检测、session-first cache。
...
- **Forgetting Engine**：运行时活力值按 `VITALITY_DECAY_HALF_LIFE_DAYS` 衰减（`.env.example` 中为 30 天）。低于阈值的记忆成为归档候选。实际归档需要 review token —— 不会静默删除。
...
四种档位（A/B/C/D），从纯本地到远程 API。Profile B 默认开启 RRF 融合；C/D 默认关闭 RRF/MMR，并在安装了 `sqlite-vec` 时走 sqlite-vec 原生向量搜索。embedding 维度变化时，vec0 KNN 表会自动 DROP 并按新维度重建，但不同 backend/model/dimension 下已经写入的旧向量仍需要 `rebuild_index(wait=true)` 或分库验证。最完整的端到端验证仍是 `macOS + Docker`；原生 Windows 通过 `backend/mcp_wrapper.py` 也已打通。远程与 GUI 宿主组合请按目标环境再做一次复核。
...
按页面拆开的使用说明：[中文仪表盘使用指南](docs/DASHBOARD_GUIDE.md)。
