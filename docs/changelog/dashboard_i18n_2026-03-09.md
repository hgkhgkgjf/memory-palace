...
前端会先恢复已保存语言；没有保存值时，中文浏览器语言映射到 `zh-CN`，其它语言默认 English。右上角支持一键切换英文 / 中文，切换后会持久化。

---

## 2. 用户能直接感知到的变化

- 已保存语言优先生效；无保存值时按浏览器语言选择初始语言
- 右上角新增语言切换按钮
- 切到中文后，常见静态文案会跟着切换
- 常见日期 / 数字格式会跟着当前语言切换
...
- 默认语言：优先恢复已保存语言；没有保存值时，中文浏览器归并到 `zh-CN`，其它场景回退英文
- 语言切换：英文 / 中文
...
- 历史上那个 `docs.skills_mcp_contract` 的误报已在发布前修正，不再是这次 i18n 结论里的阻断项
