...
>
> **A boundary note**: the current repository now has two repo-local MCP launch paths. Native Windows defaults to `backend/mcp_wrapper.py`; macOS / Linux / `Git Bash` / `WSL` still use `scripts/run_memory_palace_mcp_stdio.sh`. Both paths prefer the current repository's local `.env` and local `backend/.venv`. So the shell examples below should still be read as the POSIX path first; if you are on native Windows, prefer the MCP binding generated by the install scripts instead of treating the `bash` examples as "directly available in PowerShell." `install_skill.py` now also makes that split from the actual host boundary instead of treating every Windows-adjacent shell as the same case, so `Git Bash` / `WSL` still stay on the bash wrapper path.
>
> That boundary no longer lives only inside `install_skill.py`. `render_ide_host_config.py`, the repo-local `stdio` validation path, and the launcher guidance in this quick start now follow the same rule too: only **real native Windows** switches to `python-wrapper`; `Git Bash` / `WSL` / MSYS / Cygwin still stay on the POSIX-shell side.
>
> **One more Docker-specific clarification**: this repo-local MCP wrapper only serves the "current checkout + local `.env` + local `backend/.venv`" path. It does **not** reuse Docker container data under `/app/data`. If `.env` is missing while `.env.docker` exists, it refuses to fall back to `demo.db`; if you copy the `DATABASE_URL` from `.env.docker` into local `.env`, or an explicit `DATABASE_URL` still points to `/app/...` or `/data/...` after normalizing common slash and case variants (for example `sqlite+aiosqlite://///app/data/...` or an uppercase `/APP/...` form), it also refuses to start and tells you to use a host absolute path or the exposed Docker `/sse` endpoint instead.
>
