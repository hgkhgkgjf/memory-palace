...
>
> If you only use GHCR / Docker to start `Dashboard / API / SSE`, this is not your first entry point. Continue here only when you also want local clients to bind to this checkout.
>
...
>
> This longer page is for manual repo-local wiring, step-by-step verification, and troubleshooting. For the fastest path, start with `memory-palace-setup` or `SKILLS_QUICKSTART_EN.md`.
>
> The repo-local MCP launcher rule has two branches: native Windows uses `backend/mcp_wrapper.py`;
> macOS / Linux / `Git Bash` / `WSL` / MSYS / Cygwin use `scripts/run_memory_palace_mcp_stdio.sh`.
> `install_skill.py`, `render_ide_host_config.py`, and repo-local validation all generate config from that same rule.
>
> The repo-local wrapper only serves the "current checkout + local `.env` + local `backend/.venv`" path.
> It does not reuse Docker container data under `/app/data`. If only `.env.docker` exists,
> or local `DATABASE_URL` still points at `/app/...` / `/data/...`, the wrapper refuses to start
> and tells you to use a host absolute path or Docker `/sse`.
>
...
- If a config file or skill directory on Windows is only locked briefly, `install_skill.py` now also gives the final `replace` / promote / rollback step a small bounded retry, so the install is less likely to stop in a half-swapped state.
...
If this `workspace --check` passes, the intended reading is simple:

- `Claude`: `SKILL OK` + `MCP OK`
- `Gemini`: `SKILL OK` + `MCP OK`
- `Codex/OpenCode`: if you include them in the same workspace check, skill is still `OK`, but MCP is only a `NOTE`, because workspace scope does not define a stable repo-local MCP surface for them
...
- Also refuses to start when `.env` or an explicit `DATABASE_URL` still points to `/app/...` or `/data/...` after normalizing common slash and case variants, because those paths only exist inside the container.
...
- Refuse to start if `.env` or an explicit `DATABASE_URL` still points to `/app/...` or `/data/...` container paths after normalizing common slash and case variants.
...
This script follows the same repo-local wrapper path that users actually connect to, and it covers wrapper behavior plus
`compact_context` gist persistence. Treat the generated local report as the source of truth for your machine;
native Windows / native Linux host runtime still needs target-environment rechecks.
